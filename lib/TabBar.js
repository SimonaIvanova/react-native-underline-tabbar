...
function createTranslateXScaleX(scaleXFactor, x,centerIndicator,indicatorWidth) {
  // prettier-ignore
  const scale = centerIndicator?1:scaleXFactor
  const xPosition = centerIndicator?(x+(scaleXFactor/2) -
   (indicatorWidth && indicatorWidth>0? indicatorWidth/2:0)):x 
  return [
    scale, 0, 0, 0, // perspactive, rotation, rotation x, rotation y
    0, 1, 0, 0, // rotation z, scale, scale x, scale y 
    0, 0, 1, 0, // translate x, translate y, 
    xPosition, 0, 0, 1,
  ];
}
...
class TabBar extends Component<Props, State> {
  ...
  applyTransformToUnderline(scaleXFactor: number, dx: number) {
    ...
    const matrix = createTranslateXScaleX(scaleXFactor, dx,this.props.centerIndicator,this.props.underlineWidth);
  ...
  renderUnderline() {
    const underlineView = this.props.underlineView;
    ...
    return (
        <View ref={this.setUnderlineRef} style={[tabUnderlineStyle]}>
            { underlineView() }
        </View>   
    );   
  }
