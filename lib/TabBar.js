...
function createTranslateXScaleX(scaleXFactor, x,centerIndicator) {
  // prettier-ignore
  const scale = centerIndicator?1:scaleXFactor
  const xPosition = centerIndicator?(x+(scaleXFactor/2)):x
  return [
...
class TabBar extends Component<Props, State> {
  ...
  applyTransformToUnderline(scaleXFactor: number, dx: number) {
    ...
    const matrix = createTranslateXScaleX(scaleXFactor, dx,this.props.centerIndicator);
